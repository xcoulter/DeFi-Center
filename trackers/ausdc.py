...
# Ethereum mainnet Aave v3 default counterparties
...
# ─────── Underlying ERC-20 flows vs counterparties (helper) ───────
def _underlying_flows_wallet_vs_counterparties(
    ...
    for l in logs_from:
        ...
        if frm == wallet.lower() and to in cps:
            to_cp += _int_hex_safe(l.get("data"))

    for l in logs_to:
        frm, to = _topics_to_addresses(l.get("topics"))
        if to == wallet.lower() and frm in cps:
...
def get_atoken_interest_range(
    ...
    decimals: int = 18,             # aToken decimals (== underlying decimals for Aave)
    # NEW: use underlying flows to compute deposits/withdrawals
    underlying_token: Optional[str] = None,          # ERC-20 address (USDC for aUSDC, WETH for aWETH, ...)
    underlying_decimals: Optional[int] = None,
    counterparties: Optional[Iterable[str]] = None,  # extra addresses to treat as Aave I/O
    include_default_aave_eth_v3: bool = True,
) -> pd.DataFrame:
    """
    Compute daily interest in underlying units using:
      - aToken.balanceOf for start/end (already in underlying units)
      - UNDERLYING ERC-20 transfers wallet<->Aave as deposits/withdrawals

    interest = (end_balance - start_balance) + (withdrawals - deposits)
             = (end_balance - start_balance) + (from_Aave - to_Aave)

    Returns columns (all in 'decimals' units):
      date, start_block, end_block,
      start_balance, end_balance,
      deposits, withdrawals, net_transfers, daily_interest
    """
    infura_url = (infura_url or os.getenv("INFURA_URL","")).strip()
    if not infura_url:
    ...
    for d in _iterate_days(start_dt, end_dt):
        ...
        # aToken balances (already underlying-equivalent, in BAL_DEC units)
...
def get_underlying_flows_range(
    ...
    infura_url: Optional[str] = None,
    decimals: int = 18,
    counterparties: Optional[Iterable[str]] = None,
    include_default_aave_eth_v3: bool = True,
) -> pd.DataFrame:
    """Per-day UNDERLYING flows vs counterparties (for transparency/QA)."""
    infura_url = (infura_url or os.getenv("INFURA_URL","")).strip()
    if not infura_url:
        raise RuntimeError("Missing INFURA_URL")
    ...
    for d in _iterate_days(start_dt, end_dt):
        ...
        rows.append({
            ...
            "start_block": start_blk,
            "end_block": end_blk,
            "to_counterparties":       to_cp_u   / UNIT,  # wallet -> Aave (deposit)
            "from_counterparties":     from_cp_u / UNIT,  # Aave -> wallet (withdrawal)
            "net_with_counterparties": (from_cp_u - to_cp_u) / UNIT,
        })

    return pd.DataFrame(rows)

# ─────────── Legacy fallback (mint/burn) ───────────
def _deposits_withdrawals_by_mint_burn(
    infura_url: str, token: str, wallet: str, start_block: int, end_block: int
) -> Tuple[int, int]:
    """Fallback if no underlying_token supplied: mints/burns on the aToken."""
    wtopic = _addr_topic(wallet)
    dep_logs = _get_logs_chunked(infura_url, token, start_block, end_block,
                                 [TRANSFER_SIG, ZERO_ADDR_TOPIC, wtopic],
                                 stop_at_first=False, chunk_size=50_000) or []
